...
import { createUserWithEmailAndPassword, googleSingIn, initializeLoginFramework, signInWithEmailAndPassword, handleSignOut, facebookSingIn, gitHubSingIn, } from './LoginManager';
...
const Login = () => {
    ...
    return (
        <>
            {
                loggedInUser.email ? <div className='container mt-5 col-md-3 text-center'> 
                <h3>{loggedInUser.name}</h3>  <h3>Email: {loggedInUser.email}</h3> 
                ...
                <button className='btn btn-success w-100 my-1' onClick={singOUt}>Sign Out</button>
                    ...
                    : <>
                        <div className="col-md-3 mt-5 container">
                            ...
                            <button className='btn btn-dark w-100 my-1' onClick={handleGitSingIn}>GitHub Sing In</button>
